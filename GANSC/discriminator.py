...
def discriminator(self, wave_in, reuse=False):
        ...
        with tf.variable_scope('d_model') as scope:
            ...
            def disc_block(block_idx, input_, kwidth, nfmaps, laynorm, activation,
                           pooling=2):
                with tf.variable_scope('d_block_{}'.format(block_idx)):
                    ...
                    if input_.get_shape().as_list()[2] == nfmaps:
                        pooling = 1
                    else:
                        pooling = 2
                    ...
                    hi_a = downconv(input_, nfmaps, kwidth=kwidth, pool=pooling,
                                    init=downconv_init, bias_init=bias_init, lnorm=laynorm)
                    if not reuse:
                        print('downconved shape: {} '
                              ''.format(hi_a.get_shape()), end=' *** ')
                    #if self.loss_type == 'wasserstein' or self.loss_type == 'mixed_loss':
                    if laynorm:
                        if not reuse:
                            print('Applying Layer_Norm', end=' *** ')
                    #elif self.loss_type == 'l1_adv_loss':
                    else:
                        if not reuse:
                            print('Applying BN', end=' *** ')
            ...
            for block_idx, fmaps in enumerate(self.d_num_fmaps):
                hi = disc_block(block_idx, hi, kwidth,
