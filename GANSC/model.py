...
class SEGAN(Model):
    ...
    def build_model(self, config):
        all_d_grads = []
        all_g_grads = []
        #self.g_lr = tf.Variable(config.g_learning_rate, name='g_lr')
        #self.d_lr = tf.Variable(config.d_learning_rate, name='d_lr')
        #self.g_lr_update = tf.assign(self.g_lr, tf.maximum(self.g_lr * 0.5, 0.00002),
         #                            name='g_lr_update')
        #self.d_lr_update = tf.assign(self.d_lr, tf.maximum(self.d_lr * 0.5, 0.00002),
          #                           name='d_lr_update')
    ...
    def build_model_single_gpu(self, gpu_idx, loss_type):
        ...
        if gpu_idx == 0:
            ...
            #                                               self.canvas_size],
            #                                  name='sample_wavs')
            ref_Gs = self.generator(noisybatch, wavbatch, is_ref=True,
                                    spk=None,
                                    do_prelu=do_prelu)
        ...
        G, z, error  = self.generator(noisybatch, wavbatch, is_ref=False, spk=None,
                               do_prelu=do_prelu)
        self.Gs.append(G)
        self.zs.append(z)
        self.es.append(error)
        ...
        if loss_type == 'l1_adv_loss':
            ...
            #fft_error = tf.get_variable(name='error', shape=[1], initializer=tf.zeros_initializer())
            '''
            #fft_error = 0.
            for i in range(0, G_output_nidm[0]):
                ...
                fft_error = tf.add(tf.sqrt(tf.reduce_mean(tf.squared_difference(G_fft_abs, wav_fft_abs))),fft_error)
                #self.lsd_error.append(tf.sqrt(tf.reduce_mean(tf.squared_difference(G_fft_abs, wav_fft_abs))))

            lsd_error = tf.div(fft_error,G_output_nidm[0])
            #lsd_error = fft_error/G_output_nidm[0]
            '''
            ...
            self.lsd_error_sum = scalar_summary("lsd_error", error)
            ...
            #d_rl_logits = discriminator(self, D_rl_joint, reuse=True)
            # build fk G discriminator
            #d_fk_logits = discriminator(self, D_fk_joint, reuse=False)

            #X_real = D_rl_joint
            #X_fake = D_fk_joint
            d_logits_real = discriminator(self, D_rl_joint, reuse=True)
            d_logits_fake = discriminator(self, D_fk_joint, reuse=True)

            # make disc variables summaries
            #self.d_rl_sum = histogram_summary("d_real", d_rl_logits)
            self.d_fk_sum = histogram_summary("d_fake", d_logits_fake)
            # self.d_nfk_sum = histogram_summary("d_noisyfake", d_nfk_logits)
            ...
            # WGAN Loss
            g_adv_loss = -tf.reduce_mean(d_logits_fake)
            #g_adv_loss = tf.reduce_mean(tf.squared_difference(d_logits_fake, 1.))
            g_l1_loss = self.l1_lambda * tf.reduce_mean(tf.abs(tf.sub(G,
                                                                      wavbatch)))
            # g_l1_loss = self.l1_lambda * tf.reduce_mean(tf.squared_difference(G, wavbatch))
            #g_loss = g_adv_loss + 1e-4*g_l1_loss #g_adv_loss +
            g_loss =  g_adv_loss + g_l1_loss  #g_adv_loss +
            #d_rl_loss = tf.reduce_mean(tf.squared_difference(d_logits_real, 1.))
            #d_fk_loss = tf.reduce_mean(tf.squared_difference(d_logits_fake, 0.))
            #d_loss = d_rl_loss + d_fk_loss
            ...
            epsilon = tf.random_uniform(
                ...
                minval=0.,
                maxval=1.)
            X_hat = D_rl_joint + epsilon * (D_fk_joint - D_rl_joint)
    ...
    def train(self, config, devices, mode):
        ...
        if config.loss_type == 'l1_adv_loss':
            ...
            if mode == 'stage1':
                ...
                load_model_path = os.path.join((config.save_path).split('/')[0], (config.save_path).split('/')[1],'stage1',
                                               (config.save_path).split('/')[3])
            ...
            sample_z, sample_error = self.sess.run([self.gtruth_noisy[0],
                                      self.gtruth_wavs[0],
                                      self.zs[0],self.es[0]])
            ...
            try:
                while not coord.should_stop():
                    ...
                    if counter % config.save_freq == 0:
                        ...
                        _g_opt, _g_sum, \
                        g_adv_loss, \
                        g_l1_loss,lsd_error, g_loss = self.sess.run([g_opt, self.g_sum,
                                                           self.g_adv_losses[0],
                                                           self.g_l1_losses[0], self.lsd_errors[0], self.g_losses[0]])
                    else:
                        for d_iter in range(self.disc_updates):
                            _d_opt, \
                            d_fk_loss, \
                        ...
                        _g_opt, \
                        g_adv_loss, \
                        g_l1_loss, lsd_error, g_loss = self.sess.run([g_opt, self.g_adv_losses[0],
                                                           self.g_l1_losses[0], self.lsd_errors[0], self.g_losses[0]])
                    ...
                    if (counter / num_devices) % config.save_freq == 0:
                        ...
                        fdict = {self.gtruth_noisy[0]: sample_noisy, self.gtruth_wavs[0]: sample_wav,
                                 self.zs[0]: sample_z, self.es[0]: sample_error}
            ...
            print('sample wav shape: ', sample_wav.shape)
            print('sample z shape: ', sample_z.shape)
            #print('sample lsd error shape:', sample_error)
                    ...
                    if (counter / num_devices) % config.save_freq == 0:
                        ...
                        for m in range(min(20, canvas_w.shape[0])):
                            ...
                            np.savetxt(os.path.join(save_path, 'g_losses.txt'),
                                       g_losses)
                            np.savetxt(os.path.join(save_path, 'lsd_errors.txt'),
                                       lsd_errors)
    ...
    def clean(self, x):
        ...
        for beg_i in range(0, x.shape[0], self.canvas_size):
            ...
            canvas_w = self.sess.run(self.Gs[0],
                                     feed_dict=fdict)[0]
            t2 = time.time()
            print('time to process a 1s segment:{}'.format(t2-t1))
