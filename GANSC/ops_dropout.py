...
def conv2d(batch_input, out_channels, filter_shape, strides, name="conv"):
    with tf.variable_scope(name):
        ...
        #tf.pad(): https://blog.csdn.net/zhang_bei_qing/article/details/75090203
...
def GaussianLogDensity(x, mu, log_var, name='GaussianLogDensity'):
    with tf.name_scope(name):
        ...
        x_mu2_over_var = tf.div(x_mu2, var + tf.keras.backend.epsilon()) # use tf.keras.backend.epsilon()=1e-07?
